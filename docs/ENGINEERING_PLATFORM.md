...
The legacy platform walkthrough below remains useful for contributor orientation. Separately, the published Gen2 governed progression establishes immutable evidence, durable transition control, and one narrowly bounded local construction authority:
...
→ Construction Work-Order Evidence → Bounded Workspace File Construction
→ Fixed-Profile Terminal Presentation → Loopback Visual Presentation
→ Browser-Local Manifest and Result Conveniences → Cohesive Static Workflow Hierarchy
```

G2.4.14 through G2.4.32 are complete and published. G2.4.22 was published at `14f42717be5e819f41ee0369e93417303b5753b3` under immutable tag `v2.4.22-g2.4.22`; G2.4.23 through G2.4.32 follow under their corresponding immutable tags, with G2.4.32 at commit `cc23dee6fcb09e3544fdffce56a7b2c19cc028bf` under `v2.4.32-g2.4.32`. G2.4.17 remains the sole durable, fail-closed pre-execution transition-control ledger for its existing external-artifact-transition profile. G2.4.18 remains immutable destination-contract evidence only, G2.4.19 remains immutable outcome-semantics policy evidence only, and G2.4.20 remains declared destination-contract attestation-policy evidence only. G2.4.21 remains immutable construction work-order evidence with typed exact request/assessment provenance; it does not itself write files.

G2.4.10 remains the sole owner of custody acquisition, live descriptor continuity, process-local issued-handle provenance, and handle lifecycle. G2.4.22 supplies the exact immutable G2.4.21 work-order chain, validates the construction plan and required primitives, then consumes one matching published G2.4.10 handoff without reopening a root pathname, reconstructing a descriptor, or verifying private issuance state. Its sole effect authority is descriptor-relative creation of declared new UTF-8 text files using exclusive and no-follow operations.

G2.4.22 returns immutable receipts only for the completed ordered file prefix. Definitive pre-create failure is `CONSTRUCTION_REFUSED` with no receipt, while an effect-started post-create failure is `PARTIAL_CONSTRUCTION_STOPPED` with no receipt for the unsuccessful action. First failure is terminal; no retry, rollback, cleanup, recovery, or reconciliation exists. EBS-037 final acceptance proves the direct effect states, receipt truthfulness, pathname-replacement continuity, capability absence, and rejected handoff states. G2.4.22 has no root-provisioning, overwrite, delete, move, copy, chmod, command, build, dependency, browser, network, credential, session, permit, ledger, publication, release, or deployment authority. The B5/B6 evidence vocabulary remains truthful: `OBSERVED_ZERO_EFFECT_CATEGORIES=NONE`; unavailable operational categories are `CAPABILITY_ABSENT`; and immutable evidence, receipts, and test-owned filesystem state use `DIRECT_STATE_PROOF`. G2.4.23 through G2.4.32 are complete and published. G2.4.23 is the thin Typer terminal boundary; G2.4.24 is the loopback-only standard-library WSGI boundary; G2.4.25 preserves representation-only import and exact literal relay; G2.4.26–G2.4.31 add browser-local loading, discard, clearance, and dismissal conveniences; and G2.4.32 is the current semantic presentation layer. G2.4.32 preserves the frozen `app.js` behavior, keeps Create as the sole submission transition, and presents four stages: Prepare locally, Review prepared inputs, Explicitly submit governed construction, and Review terminal result. EBS-048 directly proves receipt-backed success and upstream refusal with nonempty facts and zero receipts through real response rendering and dismissal. No new network, persistence, runtime, build, test, deployment, browser-automation, or generalized application-generation authority was introduced. G2.4.33 is not started.
