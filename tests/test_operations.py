...
class TestOperationRegistry:
    ...
    def test_find_unsupported_raises(
        self, registry: OperationRegistry, analyzer: GoalAnalyzer
    ) -> None:
...
class TestBuiltInOperations:
    def test_rename_symbol_generates_tasks(
        self, registry: OperationRegistry, analyzer: GoalAnalyzer
    ) -> None:
        goal = analyzer.analyze(PlanningGoal(goal_type=GoalType.REFACTOR, title="Rename X"))
        op = registry.find(goal)
    ...
    def test_refactor_generates_tasks(
        self, registry: OperationRegistry, analyzer: GoalAnalyzer
    ) -> None:
