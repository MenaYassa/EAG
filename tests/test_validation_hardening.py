...
def build_broken_artifact(
    ...
    for t in tasks:
        nodes[t.id] = TaskDependencyNode(
            task=t, 
            incoming=tuple(incoming_map.get(t.id, [])), 
            outgoing=t.dependencies
        )
...
class TestMultipleIssues:
    def test_multiple_validation_issues_collected(self, validator: PlanValidator) -> None:
        ...
        assert any(i.category == ValidationCategory.DEPENDENCY and i.severity == ValidationSeverity.ERROR for i in result.issues)
        assert any(i.category == ValidationCategory.SAFETY and i.severity == ValidationSeverity.CRITICAL for i in result.issues)
