...
def _admit(gate: ControlledRuntimeSessionGate, bindings):
...
def test_ebs_023_durable_session_replay_ledger_is_cross_context_fail_closed_and_nonexecuting(
    ...
    gate_a = ControlledRuntimeSessionGate(replay_ledger=durable_ledger(success.control_root))
    session = _admit(gate_a, success)
    first_start = _consume(gate_a, session, success)

    gate_b = ControlledRuntimeSessionGate(replay_ledger=durable_ledger(success.control_root))
    ...
    corrupt_session = _admit(corrupt_gate, corrupt)
    ...
    conflict_result = ControlledRuntimeSessionGate(replay_ledger=conflict_ledger).create_session(
