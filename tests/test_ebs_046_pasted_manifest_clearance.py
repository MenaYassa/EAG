"""EBS-046 — browser-local pasted-manifest clearance over the published visual boundary."""
...
def _run_real_pasted_clearance_listener(tmp_path: Path, scenario: str) -> dict[str, object]:
    runner_path = tmp_path / "ebs_046_pasted_manifest_clearance_runner.mjs"
...
globalThis.fetch = () => {
  requestCount += 1;
  throw new Error('EBS-046 pasted-manifest clearance must not issue a request');
};
...
def _run_with_unchanged_workspace(tmp_path: Path, *, identity: str, scenario: str) -> dict[str, object]:
    ...
    result = _run_real_pasted_clearance_listener(tmp_path, scenario)
...
def test_ebs_046_pasted_manifest_clearance_consumes_only_pasted_local_text(tmp_path: Path) -> None:
    result = _run_with_unchanged_workspace(tmp_path, identity="ebs-046-success", scenario="selected")
    ...
    assert result["beforePreparation"]["pastedManifest"]["value"] == '{"prepared":true}'
...
def test_ebs_046_empty_pasted_manifest_is_a_real_listener_strict_local_no_op(tmp_path: Path) -> None:
    result = _run_with_unchanged_workspace(tmp_path, identity="ebs-046-empty", scenario="empty-pasted-manifest")
...
def test_ebs_046_missing_or_invalid_clearance_targets_fail_closed_before_mutation(tmp_path: Path) -> None:
    for scenario in (
...
def test_ebs_046_missing_or_wrong_type_clear_control_preserves_all_existing_bindings(tmp_path: Path) -> None:
...
def test_ebs_046_clearance_delta_has_no_prohibited_browser_or_submission_capability() -> None:
    ...
    binding = client[
        client.index("if (clearPastedManifestButton instanceof HTMLButtonElement)") : client.index(
