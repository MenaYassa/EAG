...
class TestBatchExecutor:
    def test_execute_empty_batch(
        self, event_bus: MockEventBus, manager: WorkerManager, worker_runtime: WorkerRuntime, context: WorkerContext
    ) -> None:
        executor = BatchExecutor(event_bus=event_bus, manager=manager, worker_runtime=worker_runtime)
    ...
    def test_execute_single_node_failure(
        self, registry: WorkerRegistry, event_bus: MockEventBus, health_manager: WorkerHealthManager, context: WorkerContext
    ) -> None:
        registry.register(MockWorker(WorkerProfile(id="w_fail", name="Fail", capabilities=("python",)), fail=True))
        mgr = WorkerManager(registry=registry, health_manager=health_manager)
    ...
    def test_fail_fast_stops_on_failure(
        ...
        executor = BatchExecutor(
            event_bus=event_bus, manager=mgr, worker_runtime=wrt, 
...
class TestParallelExecutionRuntime:
    def test_execute_empty_graph(
        ...
        rt = ParallelExecutionRuntime(event_bus=event_bus, manager=manager, worker_runtime=worker_runtime)
    ...
    def test_execute_linear_graph(
        ...
        graph = ExecutionGraph(nodes=(n1, n2, n3), edges=(make_edge("n1", "n2"), make_edge("n2", "n3")))
        completed, failed = rt.execute(graph, context)
...
class TestArtifactGraph:
    ...
    def test_graph_immutable_edges(self) -> None:
        n1, n2 = ArtifactNode(id="a1", path="m", creator_worker_id="w1"), ArtifactNode(id="a2", path="t", creator_worker_id="w2")
    ...
    def test_large_artifact_graph(self) -> None:
        nodes = tuple(ArtifactNode(id=f"a{i}", path=f"file{i}.py", creator_worker_id="w1") for i in range(100))
    ...
    def test_graph_with_no_edges(self) -> None:
        nodes = tuple(ArtifactNode(id=f"a{i}", path=f"f{i}", creator_worker_id="w1") for i in range(3))
        g = ArtifactGraph(nodes=nodes)
        assert len(g.nodes) == 3
        assert len(g.edges) == 0

    def test_add_multiple_nodes(self) -> None:
        g = ArtifactGraph().add_node(ArtifactNode(path="m", creator_worker_id="w1")).add_node(ArtifactNode(path="t", creator_worker_id="w2"))
        assert len(g.nodes) == 2

    def test_add_multiple_edges(self) -> None:
        n1, n2, n3 = (ArtifactNode(id=f"a{i}", path=f"f{i}", creator_worker_id="w1") for i in range(1, 4))
        g = ArtifactGraph(nodes=(n1, n2, n3))
        g = g.add_edge(ArtifactEdge(source="a1", target="a2")).add_edge(ArtifactEdge(source="a2", target="a3"))
        assert len(g.edges) == 2
