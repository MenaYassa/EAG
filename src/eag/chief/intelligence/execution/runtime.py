...
class ExecutionRuntime:
    ...
    def execute(
        self, context: ExecutionContext, fallback_contexts: list[ExecutionContext] | None = None
    ) -> ExecutionResult:
        ...
        if fallback_contexts:
            result, report = self._fallback.execute_with_fallback(
                context, fallback_contexts, executor_func
            )

            if report.fallback_used:
                new_trace = result.trace.add_event(
                    TraceEvent(
    ...
    def _execute_provider(self, context: ExecutionContext) -> ExecutionResult:
        ...
        trace = trace.add_event(
            TraceEvent(
                type=TraceEventType.PROVIDER_SELECTED, metadata={"provider": context.provider_id}
            )
        )
