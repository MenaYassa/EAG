...
class TraitScorer:
    ...
    def score(
        self, requirements: AIRequirements, model: ModelProfile, policy: RoutingPolicy
    ) -> ScoreBreakdown:
    ...
    def _normalize_reasoning(self, level: AIReasoningLevel) -> float:
        return {"none": 0.0, "low": 0.25, "medium": 0.5, "high": 0.75, "extreme": 1.0}.get(
            level.value, 0.0
        )
    ...
    def _normalize_cost(self, cost: AICost, max_allowed_cost: AICost) -> float:
        cost_val = {"very_low": 1, "low": 2, "medium": 3, "high": 4, "very_high": 5}.get(
            cost.value, 5
        )
        max_val = {"very_low": 1, "low": 2, "medium": 3, "high": 4, "very_high": 5}.get(
            max_allowed_cost.value, 5
        )
        if cost_val == 1:
            return 1.0
        if cost_val >= max_val:
