...
class Recommender:
    ...
    def recommend(self, ranked_matches: list[CapabilityMatch]) -> CapabilityRecommendation:
        ...
        sorted_matches = sorted(ranked_matches, key=lambda m: (-m.score, m.capability.metadata.id))
        ...
        if winner.capability.metadata.estimated_risk in (
            ...
            risk_str = winner.capability.metadata.estimated_risk.value
            warnings.append(f"Selected capability carries {risk_str} risk.")
        ...
        if winner.capability.metadata.dependencies:
            warnings.append(
                f"Requires dependencies: {', '.join(winner.capability.metadata.dependencies)}"
            )
