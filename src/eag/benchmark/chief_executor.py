...
class ChiefBenchmarkExecutor:
        ...
        class MockPlanner:
            def create_plan(self, context: RunContext):
                return Plan(steps=(
                    ...
                    PlanStep(name="Create Project Structure", capability_id="workspace_create", dependencies=("step_1",), metadata={}),
                    PlanStep(name="Generate Files", capability_id="source_generate", dependencies=("step_2",), metadata={}),
                ))
    ...
    def execute(self, benchmark: Benchmark, workspace: Path) -> BenchmarkResult:
        ...
        try:
            run_result = self._runtime.execute_goal(
                context, 
                capability_runtime=self._cap_runtime
            )
