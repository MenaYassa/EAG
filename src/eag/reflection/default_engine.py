    ...
    def reflect(self, context: ReflectionContext) -> ReflectionReport:
        ...
        if review_report:
            if hasattr(review_report, 'decision') and review_report.decision != "approved":
                ...
                recommendations.append(ReflectionRecommendation(
        ...
        findings.sort(key=lambda f: (-self._SEVERITY_ORDER.get(f.severity, 0), -f.confidence, f.title))
        recommendations.sort(key=lambda r: (-self._PRIORITY_ORDER.get(r.priority, 0), -r.confidence, r.title))
