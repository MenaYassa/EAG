...
class ConflictDetector:
    ...
    def check(self, edits: list[Edit]) -> list[str]:
        ...
        for i in range(len(symbol_edits)):
            for j in range(i + 1, len(symbol_edits)):
                ...
                if e1.symbol_id == e2.symbol_id and e1.new_name != e2.new_name:
                    conflicts.append(f"Symbol conflict on {e1.symbol_id}: cannot rename to both '{e1.new_name}' and '{e2.new_name}'")
        ...
        for i in range(len(import_edits)):
            for j in range(i + 1, len(import_edits)):
                e1 = import_edits[i]
                e2 = import_edits[j]
                if e1.module == e2.module and e1.old_import == e2.old_import and e1.new_import != e2.new_import:
                    conflicts.append(f"Import conflict on {e1.module}.{e1.old_import}: cannot update to both '{e1.new_import}' and '{e2.new_import}'")
    ...
    def _overlaps(self, e1: TextEdit, e2: TextEdit) -> bool:
        ...
        if start1 < start2 < end1: return True
        if start2 < start1 < end2: return True
