"""Canonical immutable-evidence utilities for G2.4.19 outcome-semantics policy."""
...
class OutcomePolicyEvidenceError(ValueError):
    """Raised when outcome-policy evidence is structurally unsafe or inconsistent."""
...
def require_non_empty(value: object, field_name: str) -> str:
    ...
    if not isinstance(value, str) or not value.strip():
        raise OutcomePolicyEvidenceError(f"{field_name} must be a non-empty string")
    return value
...
def require_identifier(value: object, field_name: str) -> str:
    ...
    if any(character.isspace() for character in text) or "://" in text or "/" in text:
        raise OutcomePolicyEvidenceError(f"{field_name} must be a compact identifier")
    return text
...
def require_sha256(value: object, field_name: str) -> str:
    ...
    if len(text) != 64 or any(character not in "0123456789abcdef" for character in text):
        raise OutcomePolicyEvidenceError(f"{field_name} must be a lowercase SHA-256 digest")
    return text
...
def canonical_timestamp(value: object, field_name: str) -> datetime:
    ...
    if not isinstance(value, datetime) or value.tzinfo is None:
        raise OutcomePolicyEvidenceError(f"{field_name} must be a timezone-aware datetime")
...
def canonical_json(payload: dict[str, Any]) -> str:
    ...
    try:
        ...
        raise OutcomePolicyEvidenceError("evidence payload is not canonical JSON") from error
